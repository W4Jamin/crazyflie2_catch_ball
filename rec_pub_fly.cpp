...
public:
    SubscribeAndPublish()
    {
        ...
        trajectory_msg = trajectory_msg = setPosition(0.0, 0.0, 1.0, 0.0);
        trajectory_pub.publish(trajectory_msg);
        ROS_INFO("start subscribing");
        //Topic you want to subscribe
        sub = nh.subscribe("/ball_0_pub", 1, &SubscribeAndPublish::callback, this);
    }

    trajectory_msgs::MultiDOFJointTrajectory setPosition(float x, float y, float z, double yaw) {
        Eigen::Vector3d desired_position(x, y, z-1);
        trajectory_msgs::MultiDOFJointTrajectory trajectory_msg;
    ...
    void callback(const geometry_msgs::Vector3 input)
    {
        ...
        float y = input.y;
        float z = input.z;
        trajectory_msgs::MultiDOFJointTrajectory trajectory_msg = setPosition(x, y, z, 0.2);
        trajectory_pub.publish(trajectory_msg);
    }
